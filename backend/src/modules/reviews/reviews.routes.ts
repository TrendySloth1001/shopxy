...
// Order matters: register /summary before / so the static path doesn't
// get shadowed by a future generic handler.
...
/// `/me/reviews` — every review the caller has written. Separate
/// router so the path doesn't shadow the per-product listing under
/// `/products/:id/reviews`.
